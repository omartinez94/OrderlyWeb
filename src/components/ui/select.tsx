...
/**
 * Select — single-select dropdown built on Radix.
 *
 ...
 *   - Typeahead jumps to the next matching option.
 *   - Escape closes the menu and returns focus to the trigger.
 *   - Tab closes the menu and moves focus to the next field.
 *
 * The trigger is button-like: pair with a `<Label htmlFor>` (or
 * `FormLabel`). The content uses `bg-popover` (LinOverlay) with
 * `shadow-md` so it sits above the page surface.
 */
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        ...
        "bg-surface-overlay text-ink px-3 py-2 text-sm whitespace-nowrap shadow-xs",
    ...
    >
      ...
      <SelectPrimitive.Icon asChild>
        <ChevronDownIcon className="text-ink-subtle size-4" />
    ...
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        className={cn(
          ...
          "border border-border-subtle bg-popover text-popover-foreground shadow-md",
